...
class BTLEMessage {
    ...
    isMessageCompleted() {
        ...
        //finally check if resultant message is JSON:
    ...
    getMessage() {
        ...
        for(let i = 0; i < this.messages.size; i++) {
            if(this.messages.has(i)) {
                returnValue += this.bluetoothLE.bytesToString(this.bluetoothLE.encodedStringToBytes(this.messages.get(i)));
            }
            else {
                return null;
            }
